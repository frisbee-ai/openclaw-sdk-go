...
func TestRequestManager_PendingLimit_RejectsOverLimit(t *testing.T) {
	...
	// blockSendFunc keeps the request pending: launches a goroutine that blocks
	// forever, then returns nil so SendRequest proceeds to wait on respCh.
	...
	// 3rd request must be rejected BEFORE sendFunc is called.
...
// TestRequestManager_PendingLimit_ZeroUnlimited verifies that maxPending=0 imposes no limit.
func TestRequestManager_PendingLimit_ZeroUnlimited(t *testing.T) {
	...
	mu.Unlock()
	if count != 10 {
		t.Errorf("expected 10 successful registrations, got %d", count)
	}
}
...
// at most maxPending goroutines succeed in registering.
...
func TestRequestManager_ChannelOwnership_ClearNoDoubleClose(t *testing.T) {
	...
	// Clear() should signal without panicking (no double-close)
	rm.Clear()
	...
	// Manager should still be usable after Clear.
...
func TestRequestManager_ChannelOwnership_CloseNoDoubleClose(t *testing.T) {
	...
	// Close() should signal without panicking (no double-close)
	_ = rm.Close()
