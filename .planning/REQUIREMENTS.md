...
- [ ] **FOUND-01**: Client-side rate limiting — Add `RequestRateLimiter` interface with `WithRateLimit()` option to prevent server rejection under load
- [ ] **FOUND-02**: Retry budget — Add `MaxRetries` field to `ReconnectConfig`; replace unlimited (MaxRetries=0) with sensible default of 10
...
- [ ] **FOUND-04**: Unbounded pending requests limit — Add max pending requests limit with `ErrTooManyPendingRequests` error
